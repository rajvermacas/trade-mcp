...
class StockDataProvider:
    ...
    def get_market_news(
        ...
        Args:
            query_type: Type of query - "company", "market", or "sector"
            query: Optional search query (company symbol, sector name, etc.)
            start_date: Optional start date filter
            limit: Maximum number of articles (default: 10)
            request_id: Optional request ID for tracking
            
        Returns:
            Dictionary containing success status, articles, and metadata
        """
        start_time = time.time()
        
        self.logger.info(
            f"Processing news request: {query_type} query",
            extra={
        ...
        try:
            # Validate inputs
            validation_result = self._validate_news_inputs(query_type, query, start_date, limit, request_id)
            if not validation_result["valid"]:
                self.logger.warning(
                    f"News input validation failed",
                    extra={
                        "query_type": query_type,
                        "error": validation_result["error"],
                        "mcp_request_id": request_id
            ...
            # Generate mock news data for now (GREEN phase - minimal implementation)
            articles = self._generate_mock_news_articles(query_type, query, limit)
            ...
            response = {
                ...
                "metadata": {
                    ...
                    "query_type": query_type,
                    "time_range": "last_7_days" if start_date is None else f"since_{start_date}"
                }
            }
            
            total_response_time = (time.time() - start_time) * 1000
            self.logger.info(
                f"Successfully retrieved {len(articles)} news articles in {total_response_time:.2f}ms",
                extra={
                    "query_type": query_type,
    ...
    def _validate_news_inputs(
        ...
        """Validate news input parameters."""
        # Validate query_type
        valid_query_types = ["company", "market", "sector"]
        if query_type not in valid_query_types:
            return {
                "valid": False,
                "error": {
                    "code": "INVALID_PARAMETERS",
                    "message": f"Invalid query_type '{query_type}'. Must be one of: {', '.join(valid_query_types)}",
                    "details": {
    ...
    def _generate_mock_news_articles(self, query_type: str, query: Optional[str], limit: int) -> list:
        """Generate mock news articles for testing purposes (GREEN phase implementation)."""
        # This is minimal implementation to make tests pass
        articles = []
        
        for i in range(min(limit, 5)):  # Generate up to 5 mock articles
            if query_type == "company" and query:
                title = f"{query} Reports Strong Q3 Results"
                tags = ["earnings", "company", query.lower()]
            elif query_type == "market":
                title = f"Indian Markets Show Positive Momentum in Trading Session {i+1}"
                tags = ["market", "nse", "trading"]
            elif query_type == "sector" and query:
                title = f"{query} Sector Shows Growth Potential"
                tags = ["sector", query.lower(), "growth"]
            else:
                title = f"Market News Update {i+1}"
                tags = ["general", "market"]
            
            article = {
                "title": title,
                "summary": f"Summary of news article {i+1} related to {query_type} query",
                "source": f"Financial Times {i+1}",
